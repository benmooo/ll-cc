...
import {
  ...
  COLUMNS,
  isValidSquare,
} from "~/lib/core/cc-board-utils";
import "~/style/ccboard.css";
...
// --- Default Props ---
const defaultPieceTheme = (piece: Piece) =>
  `img/xiangqipieces/wikimedia/${piece}.svg`;
const defaultBoardTheme = "img/xiangqiboards/wikimedia/xiangqiboard.svg";
...
export function XiangqiBoard(props: XiangqiBoardProps) {
  ...
  const boardLayout = createMemo(() => {
    const rows = Array.from({ length: 10 }, (_, i) =>
      orientation() === "red" ? 9 - i : i,
    );
    const cols = Array.from({ length: 9 }, (_, i) =>
      orientation() === "red" ? i : 8 - i,
    );
    return rows.map((row) =>
      cols.map((col) => `${COLUMNS[col]}${row}` as Square),
    );
  });
              ...
              <For each={row}>
                {(square) => (
                  <div
                    ...
                    data-square={square}
                    onMouseOver={() =>
                      props.onMouseoverSquare?.(square, props.position[square])
                    }
                    onMouseOut={() =>
                      props.onMouseoutSquare?.(square, props.position[square])
                    }
                    style={{
                      width: `${squareSize()}px`,
